...
resid(d$cnt, fitted(caret_knn))
  
# Residual sum of squares or RSS:  sum((actual - predicted)^2)

RSS <- function(actual, fitted) sum((actual - fitted)^2)
...
coef(lm2)[1] + coef(lm2)[2] *.45 + coef(lm2)[3]

# Data frame method (easiest):
d <- read.csv("day.csv")
d$season <- factor(d$season) #Need to ensure that the names match
d$temp <- rescale(d$temp)

lm3 <- lm(cnt ~ temp + season, data = d)

predict(lm3, newdata = data.frame(temp = .45, season = factor(2)))
...
# Extract percentiles
(low <- quantile(mtcars$wt, probs = seq(0, 1, .1))[3])
(high <- quantile(mtcars$wt, probs = seq(0, 1, .1))[9])

# Create data frame for predictions; we must take care to name the 
# columns so they match the columns in the source dataset.

# We'll use this model

lm(mpg ~ wt * hp, mtcars) %>% summary

df <- data.frame(wt = c(low, high),
                 hp = mean(mtcars$hp))
...
for(i in 1:1000){
  new <- sample_frac(mtcars, 1, replace = T)
  mod <- lm(mpg ~ wt * hp, new)
  boot_diff[i] <- predict(mod, newdata = df[2,]) - predict(mod, newdata = df[1,])
}

mean(boot_diff)
2*sd(boot_diff)

# "If, through improvements
# in materials, we were able to make heavy cars light, then 
# expected mpg would decline by about 5.75, plus or minus 1.5."