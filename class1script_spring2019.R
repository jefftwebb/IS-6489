...
# Add numeric objects just as you would numbers:
(y <- x + x) 
...
# 2. Import with code
    ...
    # However, you don't have "day.csv" in your local directory.  So the above
    # command did not work. Try this:  Download the "day.csv" dataset from
    # Canvas:  Files => Data. Move it from your downloads folder to your working
    # directory: the IS 6489 project you created above. Then use read.csv to
    # read that file from your working directory. Save the dataframe as an
    # object, d (for "data").
...
# 3. Import with Rstudio.
    ...
    # This is a tab in the environment pane, upper right. Note that this uses th
    # read_csv() function from the readr package which differs sometimes in 
    # behavior from read.csv.
...
# d contains columns of three different data types: integer (int), Factor, numeric
...
# A factor variable is just a character variable with an encoded order that is
# stored in the backgrund numerically (saves space). The season variable, 1-4,
# is encoded as an integer but should be a factor.
